...
// Ex: string: ACGGU -> $|5|U|G|G|C|A  (char will be change into int)
Node* parseDNA_withHead (FILE* input){
    int length = 0;
    int base;
    Node* inputList = newNode();
    while((base = fgetc(input)) != EOF && base != '>'){
...
FastaList* parseFile_withHead(FILE* input){
    FastaList* fastaList = newFastal();
    int number;
    int base;
    base = fgetc(input);
    ...
    loop:
    number = parseFasta(input);
    Node* content = parseDNA_withHead(input);
    fastaList = consFastal(number, content, fastaList);
    if((base = getc(input)) != EOF){
        goto loop;
    } else {
